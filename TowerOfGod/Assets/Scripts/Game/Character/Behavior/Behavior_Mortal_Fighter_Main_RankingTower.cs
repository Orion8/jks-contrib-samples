...
public class Behavior_Mortal_Fighter_Main_RankingTower : Behavior_Mortal_Fighter_Main
{
...
//		{
//			if (Knowledge.AttackCoolTimer && Knowledge.AttackCoolTimer.IsCoolingInProgress 
////			    && Knowledge.inRangeFromCam(2)
//			    )
//			{
////				if (name.Contains("M2"))
////					Log.jprint(gameObject + " ---------------       isCoolingInProgress ");
...
//	}
	...
	protected override void scan()
	{
		...
		{
			if ( Knowledge.IsLeader && 
			     (Knowledge.AI_AutoCombo_QuickSkill && Knowledge.ComboCurrent_QuickSkill == 0) ) //jks if leader and auto and quick skill started, 
			{
				if (Knowledge.Progress_SkillAction) //jks HACK for now. How AI_AutoCombo_QuickSkill and Progress_Action both be true ? .. It happens so do some hack here.
				{
					Knowledge.AI_AutoCombo_QuickSkill = false;
					return;
				}
	...
	protected override void processDeath()
	{
		if (Knowledge.AllyID != eAllyID.Ally_Human_Me)
		{
			...
			Log.jprint("----------------processDeath  "+gameObject);
